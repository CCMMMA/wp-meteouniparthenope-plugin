...
class InstrumentShortcode extends BaseShortcode {
    private static $instanceCount = 0;
    
    public function enqueueAssets() {
        wp_enqueue_script(
            'instrument-shortcode-js',
            $this->plugin_dir_url . 'static/js/shortcodes/instrument_shortcode.js',
            [],
            null,
            true
        );
    ...
    public function prepareData($atts) {
        ...
        if(isset($atts['mode'])){
            if($atts['mode']=="STANDALONE"){
                $mode = "STANDALONE";
                $variablesNames = isset($atts['variables']) ? esc_js($atts['variables']) : null;
                $variablesDescriptions = isset($atts['variables_desc']) ? esc_js($atts['variables_desc']) : null;
    ...
    protected function generateHTML($data) {
        ...
        $inlineScript = sprintf(
            'new InstrumentLiveChart({shortcode_id: "%s", instrument_id: "%s", long_name: "%s", lat: "%s", long: "%s" , mode: "%s", variablesName: %s, variablesDescription: %s});',
            $data['id'],
            $data['instrument_id'],
            $data['long_name'],
            $data['lat'],
        ...
        wp_add_inline_script('instrument-shortcode-js', $inlineScript);
